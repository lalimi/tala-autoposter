...
class WriterAgent:
    ...
    HOOK_TYPES = (
        "розкриття доходів: назви часовий проміжок і конкретну цифру продажів "
        "або суму, без розгону перед нею",
        "мінімум→максимум: зіштовхни малий вхід (одна дія, короткий час) з "
        "непропорційним результатом",
        "цінність із застереженням: віддай корисне і поясни, кому саме це не "
        "варто читати або кому це зайве. фразу вигадай свою, НЕ починай тими "
        "самими словами, що вже були в недавніх постах",
        "маленький мілстоун: одна скромна перемога, названа через проблему, яку "
        "вона закрила, а не через цифру",
        "пряме звертання до конкретної ситуації читача, у якій він себе впізнає. "
        "формулювання щоразу нове, не шаблон",
        "особиста історія: дієслово від першої особи, конкретний день і одна "
        "деталь, якої не вигадаєш",
        "пост-момент: одна сцена без жодної цифри й без висновку",
        "пост-діалог: чиясь репліка і що ти насправді подумала у відповідь",
    )
